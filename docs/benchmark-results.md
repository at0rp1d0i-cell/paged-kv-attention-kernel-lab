...
split partial/reduce correctness 与 equal-work analysis 已完成。下一步补 same-shape canonical
sweep：比较 `batch=1/2`、context `8K/16K/32K` 下的 single 与 `split=1/4/8/16`，并验证
`batch=16/32` 不因 split 回退。只有这些收益边界进入 CSV 后，才能据此实现并测试 adaptive
dispatch；equal-work 结果不能单独决定线上 dispatch policy。
