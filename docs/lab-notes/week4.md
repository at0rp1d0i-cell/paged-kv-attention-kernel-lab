...
```text
B=1:    8 programs, 约 276 GB/s, 15.4%
B=8:   64 programs, 约 1590 GB/s, 88.8%
B=16: 128 programs, 约 1681 GB/s, 93.8%
B=32: 256 programs, 约 1689 GB/s, 94.2%
```
...
当前 PyTorch 2.9.1 / CUPTI 13.0.48 的 `torch.profiler` 只记录到 CPU events，没有生成 CUDA
kernel events。隔离的 PyTorch 2.13.0 / CUPTI 13.0.85 probe 可以正常记录 GPU kernel，说明问题
位于当前 Kineto/CUPTI stack，而不是 RTX 5090、权限或脚本。因此不从旧 timeline 继承结论，
正式延迟与优化判断继续以 CUDA events 和解析带宽模型为准。
...
正式 sweep 显示，`B=1,S=16384,block=32` 时 FlashInfer 为 `0.0265 ms`，Paged
